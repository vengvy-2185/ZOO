...
export function KidsShell({ icon, title, subtitle, children }: { icon: LucideIcon; title: [string, string]; subtitle: [string, string]; children: React.ReactNode }) {
  ...
  return (
    <div className="pb-20 md:pb-0">
      <Navbar />
      <PageHeader icon={icon} eyebrow={km ? "តំបន់កុមារ" : "Kids Zone"} title={km ? title[1] : title[0]} subtitle={km ? subtitle[1] : subtitle[0]} />
      <main className="mx-auto max-w-5xl px-4 pb-12 md:px-6">
        <Link href="/kids" className="mb-4 inline-flex items-center gap-1.5 text-sm font-bold text-primary">
          <ArrowLeft size={16} /> {km ? "ត្រឡប់ទៅតំបន់កុមារ" : "Back to Kids Zone"}
        </Link>
        {children}
      </main>
      <SiteFooter />

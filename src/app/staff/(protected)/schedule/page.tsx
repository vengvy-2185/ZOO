...
export default async function SchedulePage() {
  ...
  const L = km
    ? { title: "កម្មវិធីថ្ងៃនេះ", sub: "កម្មវិធីសម្តែង និងការផ្តល់ចំណី តាមលំដាប់ម៉ោង។ ចុចធីកពេលធ្វើរួច។", now: "កំពុងដំណើរការ", next: "បន្ទាប់", in: (m: number) => `ក្នុង ${m} នាទី`, min: "នាទី", done: "រួចរាល់", mark: "ចុចពេលរួច", progress: "បានធ្វើរួច", left: "នៅសល់", animal: "រឿងសម្រាប់ប្រាប់ភ្ញៀវ" }
    : { title: "Today's programme", sub: "Shows and feedings in time order. Tick each one when it's done.", now: "On now", next: "Next", in: (m: number) => `in ${m} min`, min: "min", done: "Done", mark: "Tick when done", progress: "Done today", left: "Still to do", animal: "Facts to tell visitors" };

...
export default async function TeamAttendancePage({ searchParams }: { searchParams: { at?: string; month?: string; sess?: string; q?: string } }) {
  ...
  return (
    <StaffShell title={km ? "វត្តមានក្រុមការងារ" : "Team attendance"} subtitle={km ? "វេនព្រឹក និងរសៀល អ្នកមកយឺត ថ្ងៃសម្រាក និងសរុបប្រចាំខែ។" : "Morning and afternoon check-ins, late arrivals, days off and the monthly totals."}>
      {/* at check-in time the QR pops up full screen here too */}
      <KioskQR km={km} windows={windows} dayOff={dayOff} overlayOnly />
      <AttendanceManager tab={tab} month={searchParams.month} km={km} base="/staff/team" sess={searchParams.sess} q={searchParams.q} />
    </StaffShell>
  );
}
